...
const MessageInput = ({ user, selectedUser, onSend }) => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const msgWithTime = { ...res.data, createdAt: new Date().toISOString() };
      socket.emit("sendMessage", msgWithTime); // emit via socket
      onSend(msgWithTime); // update parent messages
      setText(""); // clear input
    } catch (err) {
      console.error("Error sending message:", err.response?.data || err.message);
    }
  ...
  return (
    <form
      onSubmit={handleSend}
      className={`d-flex align-items-center border-top p-3 shadow-sm ${darkMode ? "bg-dark" : "bg-white"}`}
      style={{
        position: "sticky",
        bottom: 0,
        borderTopLeftRadius: "1rem",
    ...
    >
      ...
      >
        <input
          type="text"
          className={`form-control border-0 py-2 ps-3 ${darkMode ? "text-white bg-dark" : "text-dark bg-light"}`}
          placeholder="Type a message..."
          value={text}
          onChange={(e) => setText(e.target.value)}
          style={{ outline: "none" }}
