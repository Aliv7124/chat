...
const ChatWindow = ({ user, selectedUser, socket }) => {
  ...
  useEffect(() => {
    ...
    const handleStatusUpdate = ({ userId, status }) => {
      if (userId === selectedUser._id) {
        ...
        else fetchLastSeen(); // fallback to last seen
      }
    };

    socket.on("updateUserStatus", handleStatusUpdate);
    return () => socket.off("updateUserStatus", handleStatusUpdate);
  }, [socket, selectedUser]);
  ...
  return (
    ...
    >
      {/* ✅ Fixed top header with online/last seen */}
      <div
        className={`d-flex align-items-center justify-content-between border-bottom px-3 py-2 ${
          darkMode ? "bg-secondary text-light" : "bg-light text-dark"
        }`}
      ...
      <form
        onSubmit={handleSend}
        className={`p-3 border-top d-flex align-items-center position-sticky bottom-0 ${
          darkMode ? "bg-secondary" : "bg-white"
        }`}
        style={{ zIndex: 10 }}
      >
