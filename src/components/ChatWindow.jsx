...
const ChatWindow = ({ user, selectedUser, socket }) => {
  ...
  const [messages, setMessages] = useState([]);
  const [text, setText] = useState("");
  const [userLastSeen, setUserLastSeen] = useState("loading");
  const [isTyping, setIsTyping] = useState(false);
  const [recording, setRecording] = useState(false);
  const [mediaRecorder, setMediaRecorder] = useState(null);
  const [audioChunks, setAudioChunks] = useState([]);
  ...
  useEffect(() => {
    ...
    const handleStatusChange = ({ userId, status, lastSeen }) => {
      if (userId === selectedUser._id) {
        ...
        else if (lastSeen) setUserLastSeen(lastSeen);
      }
    };

    socket.on("userStatusChange", handleStatusChange);

    fetchLastSeen(); // initial load
    return () => socket.off("userStatusChange", handleStatusChange);
  }, [socket, selectedUser]);
  ...
  const fetchLastSeen = async () => {
    ...
    try {
      ...
      setUserLastSeen(res.data.lastSeen || "offline");
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const handleReceive = (msg) => {
      ...
      if (sameChat) setMessages((p) => [...p, msg]);
    };
    socket.on("receiveMessage", handleReceive);
    return () => socket.off("receiveMessage", handleReceive);
  ...
  if (!selectedUser)
    return (
      ...
      >
        <h5 className="fw-semibold mb-2">Loading your friends, please wait…</h5>
        <p className="small text-muted">Fetching chat data from server…</p>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div
        className="flex-grow-1 p-3 overflow-auto"
        style={{ backgroundColor: darkMode ? "#1e1e1e" : "#f5f7fb" }}
      >
        {messages.length === 0 ? (
          <p className="text-center text-muted mt-5">
            No messages yet. Start the conversation!
      ...
      {showEmojiPicker && (
        <div
          ...
          style={{ bottom: "90px", left: "60px", zIndex: 2000 }}
        >
          <EmojiPicker
